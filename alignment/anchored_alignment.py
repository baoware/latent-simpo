...
from src.losses import infonce_loss, latent_simpo_loss, triplet_margin_loss, anchor_loss
...
def main():
    ...
    parser.add_argument("--alpha_anchor", type=float, default=0.1, help="Weight of reference anchor")
    ...
    cfg.alpha_anchor = args.alpha_anchor
    ...
    # balance weights: 50% alignment, 50% semantic anchors
    ...
    for epoch in range(cfg.epochs_alignment):
        ...
        for batch_index, batch in enumerate(progress):
            ...
            with torch.no_grad():
                win_emb = _model.forward_y_encoder(win_ids, win_mask)
                lose_emb = _model.forward_y_encoder(lose_ids, lose_mask)

                # reference model forward pass for anchor loss
